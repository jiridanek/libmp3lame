...
const PINK_REF      =          64.82;       /* 298640883795 */ /* calibration value for 89dB */


const YULE_ORDER    =     10;
const BUTTER_ORDER  =      2;
const YULE_FILTER   =  filterYule;
const BUTTER_FILTER =  filterButter;
const RMS_PERCENTILE=      0.95; /* percentile which is louder than the proposed level */
const MAX_SAMP_FREQ =  48000; /* maximum allowed sample frequency [Hz] */
const RMS_WINDOW_TIME_NUMERATOR  =  1;
const RMS_WINDOW_TIME_DENOMINATOR= 20; /* numerator / denominator = time slice size [s] */
const STEPS_per_dB  =    100; /* Table entries per dB */
const MAX_dB        =    120; /* Table entries for 0...MAX_dB (normal max. values are 70...80 dB) */

//enum {
  const GAIN_NOT_ENOUGH_SAMPLES = -24601;
  const GAIN_ANALYSIS_ERROR = 0;
  const GAIN_ANALYSIS_OK = 1;
  const INIT_GAIN_ANALYSIS_ERROR = 0;
  const INIT_GAIN_ANALYSIS_OK = 1;
//};

//enum {
  const MAX_ORDER = (BUTTER_ORDER > YULE_ORDER ? BUTTER_ORDER : YULE_ORDER);
  const MAX_SAMPLES_PER_WINDOW = ((MAX_SAMP_FREQ * RMS_WINDOW_TIME_NUMERATOR) / RMS_WINDOW_TIME_DENOMINATOR + 1); /* max. Samples per Time slice */
//};

class replaygain_t {
    var linprebuf = new typed.Float32List(MAX_ORDER * 2);
    typed.Float32List linpre;     /* left input samples, with pre-buffer */
    var lstepbuf = new typed.Float32List(MAX_SAMPLES_PER_WINDOW + MAX_ORDER);
    typed.Float32List lstep;      /* left "first step" (i.e. post first filter) samples */
    var loutbuf = new typed.Float32List(MAX_SAMPLES_PER_WINDOW + MAX_ORDER);
    typed.Float32List lout;       /* left "out" (i.e. post second filter) samples */
    var rinprebuf = new typed.Float32List(MAX_ORDER * 2);
    typed.Float32List rinpre;     /* right input samples ... */
    var rstepbuf = new typed.Float32List(MAX_SAMPLES_PER_WINDOW + MAX_ORDER);
    typed.Float32List rstep;
    var routbuf = new typed.Float32List(MAX_SAMPLES_PER_WINDOW + MAX_ORDER);
    typed.Float32List rout;
    ...
    int     freqindex;
    int     first;
    var A = new typed.Uint32List(STEPS_per_dB * MAX_dB);
    var B = new typed.Uint32List(STEPS_per_dB * MAX_dB);
...
var ABYule = new Array2D<typed.Float32List>.from([
...
var ABButter = new Array2D<typed.Float32List>.from([
...
void
filterYule(typed.Float32List input, typed.Float32List output, int nSamples, typed.Float32List kernel)
{
    /*register double  y; */
...
void
filterButter(typed.Float32List input, typed.Float32List output, int nSamples, typed.Float32List kernel)
{                       /*register double  y; */
...
int
ResetSampleFrequency(replaygain_t rgData, int samplefreq)
{
    int     i;
...
int
InitGainAnalysis(replaygain_t rgData, int samplefreq)
{
...
int
AnalyzeSamples(replaygain_t rgData, typed.Float32List left_samples, typed.Float32List right_samples,
               int num_samples, int num_channels)
{
    typed.Float32List curleft;
    typed.Float32List curright;
    int    batchsamples;
    int    cursamples;
    int    cursamplepos;
    int     i;
    ...
    while (batchsamples > 0) {
        ...
        if (rgData.totsamp == rgData.sampleWindow) { /* Get the Root Mean Square (RMS) for this set of samples */
            final double val =
                STEPS_per_dB * 10.0 * log10((rgData.lsum + rgData.rsum) / rgData.totsamp * 0.5 +
                                           1.0e-37);
            int  ival = (val <= 0) ? 0 : val;
...
double
analyzeResult(typed.Uint32List Array, int len)
{
    int elems;
    int upper;
    int sum;
    ...
    upper = ceil(elems * (1.0 - RMS_PERCENTILE)).toInt();
    sum = 0;
    for (i = len; i-- > 0;) {
        sum += Array[i];
        if (sum >= upper) {
    ...
    return PINK_REF.toDouble() - i.toDouble() / STEPS_per_dB.toDouble();
}
...
GetTitleGain(replaygain_t rgData)
{
    ...
    retval = analyzeResult(rgData.A, rgData.A.length);
