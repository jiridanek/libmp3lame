...
//
//import "src/lame.dart";
//import "src/machine.dart";
//
//import "src/encoder.dart";
//import "src/util.dart";
//import "src/lame_global_flags.dart";
//import "src/gain_analysis.dart";
//import "src/bitstream.dart";
//import "src/quantize_pvt.dart";
//import "src/set_get.dart";
//import "src/quantize.dart";
//import "src/psymodel.dart";

//import "src/VbrTag.dart";
//import "src/tables.dart";
//
//import "src/floatingpoint.dart";
//
//import "src/asmstuff.dart";
...
int
is_lame_internal_flags_valid(lame_internal_flags gfc)
{
    if (gfc == null)
        return 0;
...
class band_pass_t {
          ...
          int     lowpass;
          band_pass_t(this.bitrate, this.lowpass);
}

List
optimum_bandwidth(num lowerlimit, num upperlimit, int bitrate)
{
/*
 *  Input:
 ...
 */
    ...
    List<band_pass_t> freq_map = [
        new band_pass_t(8, 2000),
        new band_pass_t(16, 3700),
        new band_pass_t(24, 3900),
        new band_pass_t(32, 5500),
        new band_pass_t(40, 7000),
        new band_pass_t(48, 7500),
        new band_pass_t(56, 10000),
        new band_pass_t(64, 11000),
        new band_pass_t(80, 13500),
        new band_pass_t(96, 15100),
        new band_pass_t(112, 15600),
        new band_pass_t(128, 17000),
        new band_pass_t(160, 17500),
        new band_pass_t(192, 18600),
        new band_pass_t(224, 19400),
        new band_pass_t(256, 19700),
        new band_pass_t(320, 20500)
    ];
...
lame_init_qval(lame_global_flags gfp)
{
    ...
    case 0:
        ...
        cfg.full_outer_loop = 1;
        break;
    default:            /* no psymodel, no noise shaping */
        cfg.noise_shaping = 0;
        cfg.noise_shaping_amp = 0;
        cfg.noise_shaping_stop = 0;
        cfg.use_best_huffman = 0;
...
num
linear_int(num a, num b, num m)
{
    return a + m * (b - a);
}

class q_map {
  int sr_a; num qa, qb, ta, tb; int lp;
  q_map(this.sr_a, this.qa, this.qb, this.ta, this.tb, this.lp);
}
...
lame_init_params(lame_global_flags gfp)
{
    ...
    if (null == gfc.sv_rpg.rgdata)
        gfc.sv_rpg.rgdata = new replaygain_t();
    if (null == gfc.sv_rpg.rgdata) {
        freegfc(gfc);
    ...
    if (gfp.VBR == vbr_off && gfp.compression_ratio > 0) {
        ...
        if (gfp.samplerate_out == 0)
            gfp.samplerate_out = map2MP3Frequency((0.97 * gfp.samplerate_in).toInt()); /* round up with a margin of 3% */
        ...
        /* we need the version for the bitrate table look up */
         List ret = SmpFrqIndex(gfp.samplerate_out, cfg.version);
         cfg.samplerate_index = ret.first;
         cfg.version = ret.last;

        if (cfg.free_format == 0) /* for non Free Format find the nearest allowed bitrate */
    ...
    if (gfp.samplerate_out == 0 && (gfp.VBR == vbr_mt || gfp.VBR == vbr_mtrh)) {
        num qval = gfp.VBR_q + gfp.VBR_q_frac;
        var m =
          [ new q_map(48000, 0.0,6.5,  0.0,6.5, 23700)
          , new q_map(44100, 0.0,6.5,  0.0,6.5, 21780)
          , new q_map(32000, 6.5,8.0,  5.2,6.5, 15800)
          , new q_map(24000, 8.0,8.5,  5.2,6.0, 11850)
          , new q_map(22050, 8.5,9.01, 5.2,6.5, 10892)
          , new q_map(16000, 9.01,9.4, 4.9,6.5,  7903)
          , new q_map(12000, 9.4,9.6,  4.5,6.0,  5928)
          , new q_map(11025, 9.6,9.9,  5.1,6.5,  5446)
          , new q_map( 8000, 9.9,10.0, 4.9,6.5,  3952)
        ];
        for (i = 2; i < 9; ++i) {
            ...
            if (gfp.samplerate_in >= m[i].sr_a) {
                if (m[i].qa <= qval && qval < m[i].qb) {
                    final num q_ = m[i].qb-m[i].qa;
                    final num t_ = m[i].tb-m[i].ta;
    ...
    if (gfp.lowpassfreq == 0) {
        ...
        switch (gfp.VBR) {
        case vbr_off:{
                var res = optimum_bandwidth(lowpass, highpass, gfp.brate);
                lowpass = res.first;
                highpass = res.last;
                break;
            }
        case vbr_abr:{
                var res = optimum_bandwidth(lowpass, highpass, gfp.VBR_mean_bitrate_kbps);
                lowpass = res.first;
                highpass = res.last;
                break;
            }
    ...
    if (gfp.samplerate_out == 0) {
        ...
        gfp.samplerate_out = optimum_samplefreq(gfp.lowpassfreq.toInt(), gfp.samplerate_in);
    }
    if (gfp.VBR == vbr_mt || gfp.VBR == vbr_mtrh) {
...
//#endif
    ...
    case vbr_mtrh:
        {
            ...
            const cmp = const [ 5.7, 6.5, 7.3, 8.2, 10, 11.9, 13, 14, 15, 16.5 ];
    ...
    if (cfg.highpassfreq > 0) {
        ...
        if (gfp.highpasswidth >= 0)
            ...
            cfg.highpass2 = (1 + 0.00) * 2.0 * cfg.highpassfreq;
    ...
    if (cfg.lowpassfreq > 0 && cfg.lowpassfreq < (cfg.samplerate_out / 2) ) {
        cfg.lowpass2 = 2.0 * cfg.lowpassfreq;
        if (gfp.lowpasswidth >= 0) {
            ...
            cfg.lowpass1 = (1 - 0.00) * 2.0 * cfg.lowpassfreq;
        }
   ...
   *******************************************************/
    var ret = SmpFrqIndex(cfg.samplerate_out, cfg.version);
    cfg.samplerate_index = ret.first;
    cfg.version = ret.last;
    ...
    j = cfg.samplerate_index + (3 * cfg.version) + 6 * ((cfg.samplerate_out < 16000) ? 1:0);
    for (i = 0; i < SBMAX_l + 1; i++)
        gfc.scalefac_band.l[i] = sfBandIndex[j].l[i];

    for (i = 0; i < PSFB21 + 1; i++) {
        final int size = (gfc.scalefac_band.l[22] - gfc.scalefac_band.l[21]) / PSFB21;
    ...
    for (i = 0; i < PSFB12 + 1; i++) {
        final int size = (gfc.scalefac_band.s[13] - gfc.scalefac_band.s[12]) / PSFB12;
    ...
    cfg.ATH_offset_factor = powf(10.f, cfg.ATH_offset_db * 0.1);
    ...
    if (cfg.vbr == vbr_off)
        gfc.sv_enc.slot_lag = gfc.sv_enc.frac_SpF
            = ((cfg.version + 1) * 72000 * cfg.avg_bitrate) % cfg.samplerate_out;
...
lame_encode_buffer_sample_t(lame_internal_flags gfc,
                            int nsamples, typed.Uint8List mp3buf, int mp3buf_size)
{
    SessionConfig_t cfg = gfc.cfg;
    ...
    while (nsamples > 0) {
        List<typed.Float32List> in_buffer_ptr = [null, null];
        ...
        in_buffer_ptr[1] = in_buffer[1];
        {
          ...
          List ret = fill_buffer(gfc, mfbuf, in_buffer_ptr, nsamples, n_in, n_out);
          n_in = ret.first;
          n_out = ret.last;
        }
        ...
        if (cfg.findReplayGain && !cfg.decode_on_the_fly)
            if (AnalyzeSamples
                ///FIXME TODO (gfc.sv_rpg.rgdata, &mfbuf[0][esv.mf_size], &mfbuf[1][esv.mf_size], n_out,
        ...
        if (esv.mf_size >= mf_needed) {
            ...
            //TODO mfbuf[1] atd does not work, reslice? Or not, it is 2d
...
class PCMSampleType
{   static const pcm_short_type = const PCMSampleType._(0);
static const pcm_int_type = const PCMSampleType._(1);
static const pcm_long_type = const PCMSampleType._(2);
static const pcm_float_type = const PCMSampleType._(3);
static const pcm_double_type = const PCMSampleType._(4);
   final int value;
    const PCMSampleType._(this.value);
}

void
lame_copy_inbuffer(lame_internal_flags gfc,
                   List l, List r, int nsamples,
                   PCMSampleType pcm_type, int jump, double s)
{
    SessionConfig_t cfg = gfc.cfg;
    EncStateVar_t esv = gfc.sv_enc;
    typed.Float32List ib0 = esv.in_buffer_0;
    typed.Float32List ib1 = esv.in_buffer_1;
    ...
    for (i = 0; i < nsamples; i++) {
        double xl = l[i*jump];
        double xr = r[i*jump];
        double u = xl * m[0][0] + xr * m[0][1];
        double v = xl * m[1][0] + xr * m[1][1];
        ib0[i] = u;
        ib1[i] = v;
    }
}
...
lame_encode_buffer_template(lame_global_flags gfp,
                            List buffer_l, List buffer_r, int nsamples,
                            typed.Uint8List mp3buf, int mp3buf_size, PCMSampleType pcm_type, int aa, double norm)
{
...
lame_encode_buffer(lame_global_flags gfp,
                   typed.Int16List pcm_l, typed.Int16List pcm_r, int nsamples,
                   typed.Uint8List mp3buf, int mp3buf_size)
{
...
lame_encode_buffer_float(lame_global_flags gfp,
                         typed.Float32List pcm_l, typed.Float32List pcm_r, int nsamples,
                         typed.Uint8List mp3buf, int mp3buf_size)
{
...
lame_init_bitstream(lame_global_flags gfp)
{
    if (gfp.is_lame_global_flags_valid()) {
        ...
        if (gfc != 0) {
            ...
            if (gfp.write_id3tag_automatic != 0) {
                id3tag_write_v2(gfp);
            }
